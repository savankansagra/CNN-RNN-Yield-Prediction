        ...
        for i in range(Max_it):
            ...
            if i%1000==0:
                ...
                rmse_te,yhat1_te,loss_val = sess.run([RMSE,Yhat1,Tloss], feed_dict={ E_t1: Batch_X_e_te[:,0:52,:],E_t2: Batch_X_e_te[:,52*1:2*52,:],E_t3: Batch_X_e_te[:,52*2:3*52,:],
                                           ...
                                           Ybar:Ybar_te,Y_t: Batch_Y_te,Y_t_2: Batch_Y_te2,is_training:True,lr:learning_rate,dropout:0.1})
...
X[:,3:3+51*3]=(X[:,3:3+51*3] - M[:,:51*3]) / S[:, :51*3]

# Logarithematic transformation
# Apply only on third component of the weather because it has screw distribution
# X[:, 3:] = np.log(X[:, 3:])
num_rows, num_columns = X.shape
for i in range(num_rows):
    for j in range(3+51*3, 3+51*4):
        if X[i][j] == 0:
            X[i][j] = np.log(X[i][j]+1)
        else:
            X[i][j] = np.log(X[i][j])

# normalization step, z score calculation for all the variable
X[:,3+51*4:] = (X[:,3+51*4:] - M[:,51*4:]) / S[:,51*4:]
...
num_units=32  # Number of hidden units for LSTM celss
...
# store the csv file
    log_df.to_csv('logs/CNN_RNN_with_z_score_and_logarithm_normalization_with_0_point_1_dropout_while_training.csv')
